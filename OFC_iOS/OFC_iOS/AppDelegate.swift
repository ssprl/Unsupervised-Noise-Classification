...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    /*func application(_ app: UIApplication, open url: URL, options: [UIApplicationOpenURLOptionsKey : Any] = [:]) -> Bool {
        ...
        importedFile = GainsFiles.importGainsFile(url, errorString: &importedFileErrorString!)
        
        // If file was imported, set as current file and load settings
        if let gainsFile: String = importedFile {
            if let jsonObj = GainsFiles.readGainsFile(gainsFile, errorString: &importedFileErrorString!) {
