...
void
ForwardFFT(Transform* fft, float* realInput, float calibration)
{
    ...
    fft->dBSPL = 10*log10f(fft->totalPower + 1e-6) - calibration;
    temp = fft->dBSPL;
...
void equalizer(Transform* transform, float* gains, int numGains) {
    int i,k,n,m,r;
    n = numGains;
    m = 1 << n; // m = 2^n
    k = transform->points;
    ...
    for(i=0;i<k/2;i++) {
        ...
        transform->real[i] *= gains[r];
        transform->real[k-i-1] *= gains[r];
        transform->imaginary[i] *= gains[r];
        transform->imaginary[k-i-1] *= gains[r];
