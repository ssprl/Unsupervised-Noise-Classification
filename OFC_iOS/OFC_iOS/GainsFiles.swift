...
class GainsFiles {
    ...
    /// The URL of the user gains directory, at `Documents/Gains`.
    static var gainsDirectory: URL {
        get {
            return documentsDirectory.appendingPathComponent("Gains", isDirectory: true)
        }
    }
    ...
    static var ignoreBundleGains: [String] {
        get {
            ...
            do {
                let content: Data = try Data(contentsOf: GainsFiles.gainsSettingsFile)
    ...
    static var bundleGains: [String] {
        get {
            let ignoreGains: [String] = GainsFiles.ignoreBundleGains
            var gainsList: [String] = []
            if let gainsURLs: [URL] = Bundle.main.urls(forResourcesWithExtension: "json", subdirectory: "Gains") {
    ...
    static var userGains: [String] {
        get {
            var gainsList: [String] = []
            do {
                let userGainsURLs: [URL] = try FileManager.default.contentsOfDirectory(at: GainsFiles.gainsDirectory, includingPropertiesForKeys: nil, options: [])
    ...
    static var gainsFiles: [String] {
        get {
            ...
            var gainsList: [String] = GainsFiles.userGains
            gainsList.append(contentsOf: GainsFiles.bundleGains)
            return gainsList
        }
    }
    ...
    static func onLaunch() {
        ...
        if !FileManager.default.fileExists(atPath: GainsFiles.gainsDirectory.path, isDirectory: &TRUE) {
            do {
                try FileManager.default.createDirectory(at: GainsFiles.gainsDirectory, withIntermediateDirectories: false, attributes: nil)
            }
            catch let error {
                print(error)
            }
        ...
        if !FileManager.default.fileExists(atPath: GainsFiles.gainsSettingsFile.path, isDirectory: &FALSE) {
            let defaultSettingsFile: String = "{\n" +
                "\t\"ignoreBundleGains\": []\n" +
                "}\n"
            FileManager.default.createFile(atPath: GainsFiles.gainsSettingsFile.path, contents: defaultSettingsFile.data(using: .utf8), attributes: nil)
        }
    }
    ...
    static func getURL(of: String) -> URL? {
        if GainsFiles.userGains.contains(of) {
            return GainsFiles.gainsDirectory.appendingPathComponent("\(of).json")
        }
        else if GainsFiles.bundleGains.contains(of) {
            return Bundle.main.url(forResource: of, withExtension: ".json", subdirectory: "Gains")
        }
        return nil
    }
    ...
    static func readGainsFile(_ name: String, errorString: inout String) -> [String: Any]? {
        ...
        if GainsFiles.userGains.contains(name) {
            do {
                let path: URL = GainsFiles.gainsDirectory.appendingPathComponent("\(name).json")
        ...
        else if GainsFiles.bundleGains.contains(name) {
            errorString = "Could not open \(name)"
            if let file: URL = Bundle.main.url(forResource: name, withExtension: ".json", subdirectory: "Gains") {
                do {
                    errorString = "Could not read contents of \(name)"
    ...
    static func saveGainsFile(_ name: String, numGains: Int, LMHGains: [[Float]], errorString: inout String) -> String? {
        ...
        if !GainsFiles.gainsFiles.contains(saveName) {
                ...
                if LMHGains.count == 3 {
                    ...
                    if gainLen {
                        ...
                        let JSONData: String = "{\n" +
                            ...
                            "\t\t\"High\":\t\t\(gainsArrs[2])\n" +
                        "\t}\n}\n"
                        let newFileURL: URL = GainsFiles.gainsDirectory.appendingPathComponent("\(saveName).json")
                        do {
    ...
    static func deleteGainsFile(_ name: String, errorString: inout String) -> String? {
        ...
        if GainsFiles.userGains.contains(name) {
            do {
                ...
                try FileManager.default.removeItem(at: GainsFiles.gainsDirectory.appendingPathComponent("\(name).json"))
            ...
            var ignore: [String] = GainsFiles.ignoreBundleGains
            ignore.append(name)
            var JSONData: String = "{\n\t\"ignoreBundleGains\": [\n"
            ...
            do {
                ...
                try JSONData.write(to: GainsFiles.gainsSettingsFile, atomically: true, encoding: String.Encoding.utf8)
    ...
    static func importGainsFile(_ file: URL, errorString: inout String) -> String? {
        ...
        do {
            ...
            let newURL: URL = GainsFiles.gainsDirectory.appendingPathComponent("\(name).json")
            ...
            if name != "Default" && name != "Custom" {
                ...
                let inboxDir: URL = GainsFiles.documentsDirectory.appendingPathComponent("Inbox", isDirectory: true)
